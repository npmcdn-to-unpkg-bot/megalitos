...
var validator = (function(){
	...
	*/
	message = {
		...
		url				: 'URL no valido',
		number			: 'Escriba un numero',
		email			: 'Direccion de email invalido',
		email_repeat	: 'Los emails no se unen',
		password_repeat	: 'Las contraseñas no se unen',
		repeat			: 'No se unen',
		complete		: 'Entrada no completada',
	...
	*/  
    mark = function(field, text){
		...
		console.log(text);
			var item = field.siblings('ul'), warning;
        item.find('.parsley-required').remove(); 
        if( defaults.alerts ){
            warning = $('<li>').addClass('parsley-required').text( text );
            item.append( warning );
        }
	...
	*/
	unmark = function(field){
		...
		field.siblings('ul')
            .find('.parsley-required').animate({ marginLeft:32, opacity:0 }, 200, function(){
                $(this).remove();
            });
